            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int[] dir : dirs) {
                    ...
                    if (x < 0 || x >= m || y < 0 || y >= n || grid[x][y] == '#' ||
                            !reach(grid, box, player, new int[]{box[0] - dir[0], box[1] - dir[1]}) ||
                            visited.contains(Arrays.asList(x, y, x - dir[0], y - dir[1]))) continue;
                    queue.add(new int[]{x, y, x - dir[0], y - dir[1]});
                    visited.add(Arrays.asList(x, y, x - dir[0], y - dir[1]));
                }
            }
            res++;
        }
    ...
    private boolean reach(char[][] grid, int[] box, int[] player, int[] target) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int[] dir : dirs) {
                ...
                if (x < 0 || x >= m || y < 0 || y >= n || visited[x][y] || (box[0] == x && box[1] == y) || grid[x][y] == '#') continue;
