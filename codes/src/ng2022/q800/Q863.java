...
public class Q863 {
    ...
    private void helper(Map<TreeNode, Integer> map, List<Integer> res, TreeNode root, int target, int dist) {
        if (root == null) return;
        if (map.containsKey(root)) {
            dist = map.get(root);
        }
        if (dist == target) res.add(root.val);
        helper(map, res, root.left, target, dist + 1);
        helper(map, res, root.right, target, dist + 1);
    }
