...
public class Q399 {
    public double[] calcEquation(List<List<String>> equations, double[] values, List<List<String>> queries) {
        ...
        for (int i = 0; i < res.length; i++) {
            res[i] = getResult(map, queries.get(i).get(0), queries.get(i).get(1), new HashSet<String>());
        }
        return res;
    }

    private double getResult(Map<String, Map<String, Double>> map, String start, String end, HashSet<String> visited) {
        ...
        for (String next : map.get(start).keySet()) {
            ...
            visited.add(next);
            double result = getResult(map, next, end, visited);
            if (result != -1.0) return result * map.get(start).get(next);
        }
        return -1.0;
    }
